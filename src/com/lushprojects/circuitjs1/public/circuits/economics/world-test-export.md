...
World2 <- sfcr_set(
  ...
  e22 = BR ~ last(P) * BRN * lookup(BR_lookup, clamp(last(MSL), 0, 5)) * lookup(BR_lookup_2, clamp(last(CR), 0, 5)) * lookup(BR_lookup_3, clamp(last(FR), 0, 4)) * lookup(BR_lookup_4, clamp(last(POLR), 0, 60)),  # [mode=voltage, sliderValue=0 ]
  e23 = DR ~ last(P) * DRN * lookup(DR_lookup, clamp(last(MSL), 0, 5)) * lookup(DR_lookup_2, clamp(last(POLR), 0, 60)) * lookup(DR_lookup_3, clamp(last(FR), 0, 2)) * lookup(DR_lookup_4, clamp(last(CR), 0, 5)),  # [mode=voltage, sliderValue=0 ]
  e24 = NRUR ~ last(P) * NRUN * lookup(NRUR_lookup, clamp(last(MSL), 0, 10)),  # [mode=voltage, sliderValue=0 ]
  e25 = CID ~ last(CI) * CIDN,  # [mode=voltage, sliderValue=0 ]
  e26 = CIG ~ last(P) * lookup(CIG_lookup, clamp(last(MSL), 0, 5)) * CIGN,  # [mode=voltage, sliderValue=0 ]
  e27 = POLG ~ last(P) * POLN * lookup(POLG_lookup, clamp(last(CIR), 0, 5)),  # [mode=voltage, sliderValue=0 ]
  e28 = POLA ~ last(POL) / max(1e-9, lookup(POLA_lookup, clamp(last(POLR), 0, 60))),  # [mode=voltage, sliderValue=0 ]
  e29 = CFIFR ~ lookup(CFIFR_lookup, clamp(last(FR), 0, 2)),  # [mode=voltage, sliderValue=0 ]
  e30 = QLM ~ lookup(QLM_lookup, clamp(last(MSL), 0, 5)),  # [mode=voltage, sliderValue=0 ]
