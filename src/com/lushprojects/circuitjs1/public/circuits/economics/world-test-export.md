# World2 SFCR Model

## Init

```{r}
@init
  timestep: 0.2
  timeUnit: yr
  equationTableMnaMode: false
  equationTableTolerance: 0.001
  convergenceCheckThreshold: 5000
  showDots: false
  showVolts: false
@end
```

## Lookup Tables (Population & Mortality)

```{r}
@lookup BRCM
  0, 1.05
  1, 1
  2, 0.9
  3, 0.7
  4, 0.6
  5, 0.55
@end
```

## Lookup Tables (Economy, Food, Resources, Pollution, Quality)

```{r}
@lookup BRFM
  0, 0
  1, 1
  2, 1.6
  3, 1.9
  4, 2
@end
```

## Equations

```{r}
World2 <- sfcr_set(
  # [ x=50 y=50 ]
  e1 = BRN ~ 0.04,  # [mode=voltage, sliderValue=0 ]
  e2 = DRN ~ 0.028,  # [mode=voltage, sliderValue=0 ]
  e3 = CIDN ~ 0.025,  # [mode=voltage, sliderValue=0 ]
  e4 = CIGN ~ 0.05,  # [mode=voltage, sliderValue=0 ]
  e5 = NRUN ~ 1,  # [mode=voltage, sliderValue=0 ]
  e6 = POLN ~ 1,  # [mode=voltage, sliderValue=0 ]
  e7 = FC_COEFF ~ 1,  # [mode=voltage, sliderValue=0 ]
  e8 = LA ~ 1.35e8,  # [mode=voltage, sliderValue=0 ]
  e9 = PDN ~ 26.5,  # [mode=voltage, sliderValue=0 ]
  e10 = CIAFN ~ 0.3,  # [mode=voltage, sliderValue=0 ]
  e11 = ECIRN ~ 1,  # [mode=voltage, sliderValue=0 ]
  e12 = CIAFT ~ 15,  # [mode=voltage, sliderValue=0 ]
  e13 = POLS ~ 3.6e9,  # [mode=voltage, sliderValue=0 ]
  e14 = FN ~ 1,  # [mode=voltage, sliderValue=0 ]
  e15 = QLS ~ 1,  # [mode=voltage, sliderValue=0 ]
  e16 = P_0 ~ 1.65e9,  # [mode=voltage, sliderValue=0 ]
  e17 = NR_0 ~ 9e11,  # [mode=voltage, sliderValue=0 ]
  e18 = CR ~ P / (LA * PDN),  # [mode=voltage, sliderValue=0 ]
  e19 = CIR ~ CI / max(1, P),  # [mode=voltage, sliderValue=0 ]
  e20 = NRFR ~ max(0, NR) / NR_0,  # [mode=voltage, sliderValue=0 ]
  e21 = POLR ~ POL / POLS,  # Pollution ratio  [mode=voltage, sliderValue=0 ]
  e22 = BR ~ last(P) * BRN * lookup(BR_lookup, last(MSL)) * lookup(BR_lookup_2, last(CR)) * lookup(BR_lookup_3, last(FR)) * lookup(BR_lookup_4, last(POLR)),  # [mode=voltage, sliderValue=0 ]
  e23 = DR ~ last(P) * DRN * lookup(DR_lookup, last(MSL)) * lookup(DR_lookup_2, last(POLR)) * lookup(DR_lookup_3, last(FR)) * lookup(DR_lookup_4, last(CR)),  # [mode=voltage, sliderValue=0 ]
  e24 = NRUR ~ last(P) * NRUN * lookup(NRUR_lookup, last(MSL)),  # [mode=voltage, sliderValue=0 ]
  e25 = CID ~ last(CI) * CIDN,  # [mode=voltage, sliderValue=0 ]
  e26 = CIG ~ last(P) * lookup(CIG_lookup, last(MSL)) * CIGN,  # [mode=voltage, sliderValue=0 ]
  e27 = POLG ~ last(P) * POLN * lookup(POLG_lookup, last(CIR)),  # [mode=voltage, sliderValue=0 ]
  e28 = POLA ~ last(POL) / max(1e-9, lookup(POLA_lookup, last(POLR))),  # [mode=voltage, sliderValue=0 ]
  e29 = CFIFR ~ lookup(CFIFR_lookup, last(FR)),  # [mode=voltage, sliderValue=0 ]
  e30 = QLM ~ lookup(QLM_lookup, last(MSL)),  # [mode=voltage, sliderValue=0 ]
  e31 = QLP ~ lookup(QLP_lookup, POLR),  # [mode=voltage, sliderValue=0 ]
  e32 = QLF ~ lookup(QLF_lookup, FR),  # [mode=voltage, sliderValue=0 ]
  e33 = CIQR ~ lookup(CIQR_lookup, QLM / max(1e-9, QLF)),  # [mode=voltage, sliderValue=0 ]
  e34 = CIAF_D ~ CFIFR * CIQR,  # [mode=voltage, sliderValue=0 ]
  e35 = CIRA ~ CIR * CIAF / max(1e-9, CIAFN),  # [mode=voltage, sliderValue=0 ]
  e36 = FCM ~ lookup(FCM_lookup, CR),  # [mode=voltage, sliderValue=0 ]
  e37 = FPCI ~ lookup(FPCI_lookup, CIRA),  # [mode=voltage, sliderValue=0 ]
  e38 = FPM ~ lookup(FPM_lookup, POLR),  # [mode=voltage, sliderValue=0 ]
  e39 = FR ~ (FCM * FPCI * FPM * FC_COEFF) / FN,  # Food ratio  [mode=voltage, sliderValue=0 ]
  e40 = NREM ~ lookup(NREM_lookup, NRFR),  # [mode=voltage, sliderValue=0 ]
  e41 = ECIR ~ (CIR * (1 - CIAF) * NREM) / max(1e-9, (1 - CIAFN)),  # [mode=voltage, sliderValue=0 ]
  e42 = MSL ~ ECIR / ECIRN,  # Material standard of living  [mode=voltage, sliderValue=0 ]
  e43 = QLC ~ lookup(QLC_lookup, CR),  # [mode=voltage, sliderValue=0 ]
  e44 = QL ~ QLS * QLM * QLC * QLF * QLP,  # Quality of life  [mode=voltage, sliderValue=0 ]
  e45 = P_norm ~ P / P_0,  # [mode=voltage, sliderValue=0 ]
  e46 = P ~ integrate(BR - DR),  # Population (people)  [mode=voltage, sliderValue=0, initial=1.65e9 ]
  e47 = NR ~ integrate(-NRUR),  # Nonrenewable natural resources  [mode=voltage, sliderValue=0, initial=9e11 ]
  e48 = CI ~ integrate(CIG - CID),  # Capital investment  [mode=voltage, sliderValue=0, initial=4e8 ]
  e49 = POL ~ integrate(POLG - POLA),  # Pollution stock  [mode=voltage, sliderValue=0, initial=2e8 ]
  e50 = CIAF ~ integrate((CIAF_D - last(CIAF)) / CIAFT)  # Capital-investment-in-agriculture fraction  [mode=voltage, sliderValue=0, initial=0.2 ]
)
```

## Hints

```{r}
@hints
  P: Population (people)
  NR: Nonrenewable natural resources
  CI: Capital investment
  POL: Pollution stock
  CIAF: Capital-investment-in-agriculture fraction
  FR: Food ratio
  MSL: Material standard of living
  QL: Quality of life
  POLR: Pollution ratio
@end
```

## Info

```{r}
@info
# World2 (pyworld2-aligned default run)

This variant is tuned to match the default **pyworld2** standard run structure:

- timestep: 0.2 years
- default switch constants (BRN, DRN, CIDN, CIGN, FC, NRUN, POLN)
- lookup tables copied from `functions_table_default.json`
- equations follow the pyworld2 variable chain (`FR`, `MSL`, `NRMM`, `POLAT`, `CIAF` dynamics, etc.)

Reference target from pyworld2 tests at year 2100:

- QL ≈ 0.54940464789
- POLR ≈ 2.58741372815
- NR ≈ 278240023740
- CI ≈ 6010240430.13
@end
```

```{r}
@lookup BRMM
  0, 1.2
  1, 1
  2, 0.85
  3, 0.75
  4, 0.7
  5, 0.7
@end
```

```{r}
@lookup BRPM
  0, 1.02
  10, 0.9
  20, 0.7
  30, 0.4
  40, 0.25
  50, 0.15
  60, 0.1
@end
```

```{r}
@lookup DRCM
  0, 0.9
  1, 1
  2, 1.2
  3, 1.5
  4, 1.9
  5, 3
@end
```

```{r}
@lookup DRFM
  0, 30
  0.25, 3
  0.5, 2
  0.75, 1.4
  1, 1
  1.25, 0.7
  1.5, 0.6
  1.75, 0.5
  2, 0.5
@end
```

```{r}
@lookup DRMM
  0, 3
  0.5, 1.8
  1, 1
  1.5, 0.8
  2, 0.7
  2.5, 0.6
  3, 0.53
  3.5, 0.5
  4, 0.5
  4.5, 0.5
  5, 0.5
@end
```

```{r}
@lookup DRPM
  0, 0.92
  10, 1.3
  20, 2
  30, 3.2
  40, 4.8
  50, 6.8
  60, 9.2
@end
```

```{r}
@lookup CFIFR
  0, 1
  0.5, 0.6
  1, 0.3
  1.5, 0.15
  2, 0.1
@end
```

```{r}
@lookup CIM
  0, 0.1
  1, 1
  2, 1.8
  3, 2.4
  4, 2.8
  5, 3
@end
```

```{r}
@lookup CIQR
  0, 0.7
  0.5, 0.8
  1, 1
  1.5, 1.5
  2, 2
@end
```

```{r}
@lookup FCM
  0, 2.4
  1, 1
  2, 0.6
  3, 0.4
  4, 0.3
  5, 0.2
@end
```

```{r}
@lookup FPCI
  0, 0.5
  1, 1
  2, 1.4
  3, 1.7
  4, 1.9
  5, 2.05
  6, 2.2
@end
```

```{r}
@lookup FPM
  0, 1.02
  10, 0.9
  20, 0.65
  30, 0.35
  40, 0.2
  50, 0.1
  60, 0.05
@end
```

```{r}
@lookup NREM
  0, 0
  0.25, 0.15
  0.5, 0.5
  0.75, 0.85
  1, 1
@end
```

```{r}
@lookup NRMM
  0, 0
  1, 1
  2, 1.8
  3, 2.4
  4, 2.9
  5, 3.3
  6, 3.6
  7, 3.8
  8, 3.9
  9, 3.95
  10, 4
@end
```

```{r}
@lookup POLAT
  0, 0.6
  10, 2.5
  20, 5
  30, 8
  40, 11.5
  50, 15.5
  60, 20
@end
```

```{r}
@lookup POLCM
  0, 0.05
  1, 1
  2, 3
  3, 5.4
  4, 7.4
  5, 8
@end
```

```{r}
@lookup QLC
  0, 2
  0.5, 1.3
  1, 1
  1.5, 0.75
  2, 0.55
  2.5, 0.45
  3, 0.38
  3.5, 0.3
  4, 0.25
  4.5, 0.22
  5, 0.2
@end
```

```{r}
@lookup QLF
  0, 0
  1, 1
  2, 1.8
  3, 2.4
  4, 2.7
@end
```

```{r}
@lookup QLM
  0, 0.2
  1, 1
  2, 1.7
  3, 2.3
  4, 2.7
  5, 2.9
@end
```

```{r}
@lookup QLP
  0, 1.04
  10, 0.85
  20, 0.6
  30, 0.3
  40, 0.15
  50, 0.05
  60, 0.02
@end
```

```{r}
@lookup BR_lookup scope=World2
  0, 1.2
  1, 1
  2, 0.85
  3, 0.75
  4, 0.7
  5, 0.7
@end
```

```{r}
@lookup BR_lookup_2 scope=World2
  0, 1.05
  1, 1
  2, 0.9
  3, 0.7
  4, 0.6
  5, 0.55
@end
```

```{r}
@lookup BR_lookup_3 scope=World2
  0, 0
  1, 1
  2, 1.6
  3, 1.9
  4, 2
@end
```

```{r}
@lookup BR_lookup_4 scope=World2
  0, 1.02
  10, 0.9
  20, 0.7
  30, 0.4
  40, 0.25
  50, 0.15
  60, 0.1
@end
```

```{r}
@lookup DR_lookup scope=World2
  0, 3
  0.5, 1.8
  1, 1
  1.5, 0.8
  2, 0.7
  2.5, 0.6
  3, 0.53
  3.5, 0.5
  4, 0.5
  4.5, 0.5
  5, 0.5
@end
```

```{r}
@lookup DR_lookup_2 scope=World2
  0, 0.92
  10, 1.3
  20, 2
  30, 3.2
  40, 4.8
  50, 6.8
  60, 9.2
@end
```

```{r}
@lookup DR_lookup_3 scope=World2
  0, 30
  0.25, 3
  0.5, 2
  0.75, 1.4
  1, 1
  1.25, 0.7
  1.5, 0.6
  1.75, 0.5
  2, 0.5
@end
```

```{r}
@lookup DR_lookup_4 scope=World2
  0, 0.9
  1, 1
  2, 1.2
  3, 1.5
  4, 1.9
  5, 3
@end
```

```{r}
@lookup NRUR_lookup scope=World2
  0, 0
  1, 1
  2, 1.8
  3, 2.4
  4, 2.9
  5, 3.3
  6, 3.6
  7, 3.8
  8, 3.9
  9, 3.95
  10, 4
@end
```

```{r}
@lookup CIG_lookup scope=World2
  0, 0.1
  1, 1
  2, 1.8
  3, 2.4
  4, 2.8
  5, 3
@end
```

```{r}
@lookup POLG_lookup scope=World2
  0, 0.05
  1, 1
  2, 3
  3, 5.4
  4, 7.4
  5, 8
@end
```

```{r}
@lookup POLA_lookup scope=World2
  0, 0.6
  10, 2.5
  20, 5
  30, 8
  40, 11.5
  50, 15.5
  60, 20
@end
```

```{r}
@lookup CFIFR_lookup scope=World2
  0, 1
  0.5, 0.6
  1, 0.3
  1.5, 0.15
  2, 0.1
@end
```

```{r}
@lookup QLM_lookup scope=World2
  0, 0.2
  1, 1
  2, 1.7
  3, 2.3
  4, 2.7
  5, 2.9
@end
```

```{r}
@lookup QLP_lookup scope=World2
  0, 1.04
  10, 0.85
  20, 0.6
  30, 0.3
  40, 0.15
  50, 0.05
  60, 0.02
@end
```

```{r}
@lookup QLF_lookup scope=World2
  0, 0
  1, 1
  2, 1.8
  3, 2.4
  4, 2.7
@end
```

```{r}
@lookup CIQR_lookup scope=World2
  0, 0.7
  0.5, 0.8
  1, 1
  1.5, 1.5
  2, 2
@end
```

```{r}
@lookup FCM_lookup scope=World2
  0, 2.4
  1, 1
  2, 0.6
  3, 0.4
  4, 0.3
  5, 0.2
@end
```

```{r}
@lookup FPCI_lookup scope=World2
  0, 0.5
  1, 1
  2, 1.4
  3, 1.7
  4, 1.9
  5, 2.05
  6, 2.2
@end
```

```{r}
@lookup FPM_lookup scope=World2
  0, 1.02
  10, 0.9
  20, 0.65
  30, 0.35
  40, 0.2
  50, 0.1
  60, 0.05
@end
```

```{r}
@lookup NREM_lookup scope=World2
  0, 0
  0.25, 0.15
  0.5, 0.5
  0.75, 0.85
  1, 1
@end
```

```{r}
@lookup QLC_lookup scope=World2
  0, 2
  0.5, 1.3
  1, 1
  1.5, 0.75
  2, 0.55
  2.5, 0.45
  3, 0.38
  3.5, 0.3
  4, 0.25
  4.5, 0.22
  5, 0.2
@end
```

