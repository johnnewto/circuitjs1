...
class PercentElm extends CircuitElm {
    ...
    public PercentElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
        ...
        if (st.hasMoreTokens())
            clampLevel = Double.parseDouble(st.nextToken());
        setSize((f & FLAG_SMALL) != 0 ? 1 : 2);
    }
    
    String dump() {
        return super.dump() + " " + inputCount + " " + clampLevel;
    }
    ...
    void draw(Graphics g) {
        ...
        // Draw body - cyan when clamped, otherwise use standard body color (red for non-convergence)
        if (isClamped)
            g.setColor(Color.cyan);
        else
            g.setColor(getBodyColor(lightGrayColor));
    ...
    void doStep() {
        ...
        // Clamp output to +/- clampLevel
        isClamped = false;
        if (clampLevel > 0) {
            if (v0 > clampLevel) {
                v0 = clampLevel;
                isClamped = true;
            } else if (v0 < -clampLevel) {
                v0 = -clampLevel;
                isClamped = true;
            }
        }
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (n == 1)
            return new EditInfo("Clamp Level (%), 0 to disable", clampLevel).setDimensionless();
        if (n == 2)
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (n == 1) {
            clampLevel = ei.value;
            if (clampLevel < 0)
                clampLevel = 0;  // 0 means no clamping
        }
        if (n == 2) {
