...
public class OTAElm extends CompositeElm {
	...
	noDiagonal = true;
	negVolt = ((RailElm) compElmList.get(0)).getMaxVoltage();
	posVolt = ((RailElm) compElmList.get(1)).getMaxVoltage();
    }

    private void initOTA() {
	((RailElm) compElmList.get(0)).setMaxVoltage(negVolt);
	((RailElm) compElmList.get(1)).setMaxVoltage(posVolt);
    }
