...
package com.lushprojects.circuitjs1.client.electronics.misc;
...
public class ThermistorNTCElm extends CircuitElm implements Command, MouseWheelHandler {
    ...
    public ThermistorNTCElm(int xa, int ya, int xb, int yb, int f,
	    StringTokenizer st) {
	super(xa, ya, xb, yb, f);
	r25 = Double.parseDouble(st.nextToken());
	r50 = Double.parseDouble(st.nextToken());
	minTempr = Double.parseDouble(st.nextToken());
	maxTempr = Double.parseDouble(st.nextToken());
	...
	sim.getUiPanelManager().addWidgetToVerticalPanel(slider = new Scrollbar(Scrollbar.HORIZONTAL, value, 1, 0, 101, this, this));
    }
